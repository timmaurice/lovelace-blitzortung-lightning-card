...
| `map_zoom`                   | `number`  | **Optional.** The zoom level the map opens at, and stays at while there are no strikes to fit to — with `map_auto_zoom` on as well. With `map_auto_zoom: false` it is also the zoom the recenter button returns to. Range `0`–`22` (higher is closer; MapLibre caps the camera at 22).      | `13`                                        |
