...
| `map_auto_zoom`              | `boolean` | If `true`, the map fits its view to all displayed strikes. Set to `false` to keep the map at a fixed zoom instead — the recenter button then resets the view to `map_zoom` rather than re-enabling auto-zoom.                                                                               | `true`                                      |
| `map_zoom`                   | `number`  | **Optional.** The zoom level the map opens at, and returns to via the recenter button, when `map_auto_zoom` is `false`. Range `0`–`22` (higher is closer; MapLibre caps the camera at 22).                                                                                                  | `13`                                        |
